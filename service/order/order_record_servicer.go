...
type OrderRecordServicer interface {
	...
	FindOrderRecordsSortedPage(userID string, page *request.RequestPage) (*page.PageData[any], error)
	// FindOrderRecord : 현재 주문내역 리스트 조회 , 각 메뉴별 상태 변경 , / ex. 상태 : 접수중/접수취소/추가접수/접수-조리중/배달중/배달완료 등을 이용 상태 저장, 각 단계별 사업장에서 상태 업데이트 - 접수중 → 접수 or 접수취소 → 조리중 or 추가주문 → 배달중, /  성공여부 리턴
